...
function loadWalletInfo() {
  ...
  if (walletData) {
    try {
      ...
      console.log("Wallet loaded successfully:", currentWallet);
      console.log("Wallet address:", currentWallet.address);
      console.log("Wallet has private key:", !!currentWallet.privateKey);
    } catch (error) {
...
async function updateUI() {
  ...
  if (currentWallet && adapter) {
    ...
    if (manualBalance !== null) {
      ...
      document.getElementById('available-balance').textContent = manualBalance.toFixed(4);
      return;
    }
    ...
    try {
      ...
      showToast("Failed to load balance. Please use 'Set Balance' button to set your actual balance.");
    }
  } else {
...
async function confirmSend() {
  ...
  try {
    ...
    console.log("Checking balance for address:", currentWallet.address);
    ...
    if (manualBalance !== null) {
      ...
      console.log("No manual balance, checking API balance...");
      const balance = await adapter.getBalance(currentWallet.address);
      console.log("Raw balance from adapter:", balance);
      availableBalance = parseFloat(balance) / 1000000000; // MIST to SUI
    }
    
    const sendAmount = parseFloat(amount);
    
    console.log("Available balance (SUI):", availableBalance);
    console.log("Send amount (SUI):", sendAmount);
    
    if (sendAmount > availableBalance) {
      console.log("Insufficient balance detected");
      showToast(`Insufficient balance. Available: ${availableBalance.toFixed(4)} SUI`);
      return;
    }
    ...
    console.log("Balance check failed, but allowing transaction to proceed");
    showToast("Warning: Could not verify balance. Please ensure you have sufficient funds.");
  }
...
function showTransactionModal() {
  ...
  // 모달 내용 업데이트
  document.getElementById('modal-sender').textContent = pendingTransaction.from;
  document.getElementById('modal-recipient').textContent = pendingTransaction.to;
  document.getElementById('modal-amount').textContent = `${pendingTransaction.amount} ${CoinConfig.symbol}`;
  document.getElementById('modal-fee').textContent = `${pendingTransaction.fee} ${CoinConfig.symbol}`;
  document.getElementById('modal-network').textContent = CoinConfig.network.networkName;
  
  // 총 비용 계산
  const totalAmount = parseFloat(pendingTransaction.amount) + parseFloat(pendingTransaction.fee);
  document.getElementById('modal-total').textContent = `${totalAmount.toFixed(4)} ${CoinConfig.symbol}`;
...
async function executeTransaction() {
  ...
  try {
    ...
    const result = await adapter.sendTransaction(transactionData);

    console.log("Transaction result:", result);
    updateLoadingMessage("Transaction sent successfully!");
    console.log("Transaction hash:", result.hash);

    // 로딩 모달 닫기
    setTimeout(() => {
      hideLoadingModal();
      showToast(`Transaction sent successfully! Hash: ${result.hash.substring(0, 8)}...`);
...
async function requestFaucet() {
  ...
  try {
    ...
    if (result.success) {
      ...
      if (result.faucetUrl) {
        setTimeout(() => {
          const shouldOpen = confirm("Faucet API failed. Would you like to open the Sui Explorer Faucet in a new tab?");
          if (shouldOpen) {
    ...
    showToast("Faucet request failed. Please use the manual faucet.");
    
    setTimeout(() => {
      const shouldOpen = confirm("Would you like to open the Sui Explorer Faucet in a new tab?");
      if (shouldOpen) {
...
function setManualBalance() {
  const manualBalance = prompt("Enter your actual SUI balance (e.g., 1.0):", "1.0");
  
  if (manualBalance && !isNaN(parseFloat(manualBalance))) {
    // localStorage에 수동 잔액 저장
    localStorage.setItem('manual_sui_balance', manualBalance);
    
    // UI 업데이트
    document.getElementById('available-balance').textContent = parseFloat(manualBalance).toFixed(4);
...
function getManualBalance() {
  const manualBalance = localStorage.getItem('manual_sui_balance');
  if (manualBalance) {
