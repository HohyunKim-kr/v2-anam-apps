...
document.addEventListener('DOMContentLoaded', function () {
    ...
    console.log(
        'window.suiSDK.generateMnemonic:',
        window.suiSDK?.generateMnemonic
    );
    console.log(
        'window.suiSDK.Ed25519Keypair:',
        window.suiSDK?.Ed25519Keypair
    );
...
function applyTheme() {
    ...
    root.style.setProperty(
        '--coin-primary',
        CoinConfig.theme.primaryColor
    );
    root.style.setProperty(
        '--coin-secondary',
        CoinConfig.theme.secondaryColor
    );
    ...
    // 네트워크 라벨 설정
    const networkLabel =
        document.getElementById('network-label');
    if (networkLabel) {
...
async function createWallet() {
    ...
    try {
        ...
        const keypair =
            window.suiSDK.Ed25519Keypair.deriveKeypair(
                mnemonic,
                "m/44'/784'/0'/0'/0'"
            );
        console.log(
            '=== WALLET CREATION KEYPAIR DEBUG ==='
        );
        ...
        // 키페어의 내부 구조 확인
        console.log('Keypair properties:');
        for (const prop in keypair) {
            if (keypair.hasOwnProperty(prop)) {
                console.log(`  ${prop}:`, keypair[prop]);
            }
        }

        // 주소 생성
        console.log(
            '=== ADDRESS GENERATION FROM WALLET CREATION ==='
        );
        let publicKey;
        let address;
        try {
            ...
            console.log('=== GENERATED ADDRESS INFO ===');
            ...
            if (address.startsWith('0x')) {
                ...
                console.log(
                    'Hex part is valid hex:',
                    /^[a-fA-F0-9]+$/.test(hexPart)
                );
            }
        } catch (error) {
            console.error(
                'Error generating address in wallet creation:',
                error
            );
            throw error;
        }
        ...
        // 개인키 내보내기
        console.log('=== PRIVATE KEY EXPORT DEBUG ===');
        ...
        try {
            if (
                typeof window.suiSDK.exportPrivateKey ===
                'function'
            ) {
                privateKey =
                    window.suiSDK.exportPrivateKey(keypair);
                console.log(
                    'Private key exported using exportPrivateKey:',
                    privateKey
                );
            } else if (
                ...
                const secretKeyBytes =
                    keypair.exportSecretKey();
                privateKey =
                    Array.from(secretKeyBytes).join(',');
                console.log(
                ...
                console.log(
                    'Private key from export:',
                    privateKey
                );
            } else {
                console.error(
                    'No known method to export private key'
                );
                throw new Error(
                    'No known method to export private key'
                );
            ...
            console.error(
                'Error during private key export:',
                exportError
            );
            throw exportError;
        }
...
// 니모닉 확인 화면 표시
function showMnemonicConfirmation(
    mnemonic,
    address,
    privateKey
) {
...
function copyMnemonic() {
    const mnemonicElement = document.querySelector(
        '.mnemonic-display'
    );
    const mnemonic = Array.from(
        mnemonicElement.querySelectorAll('.word-text')
    )
...
function confirmMnemonic(mnemonic, address, privateKey) {
    ...
    // 니모닉 확인 화면 제거
    document
        .getElementById('mnemonic-confirmation')
        .remove();
...
async function importFromMnemonic() {
    ...
    try {
        ...
        showToast(
            'Failed to import wallet. Please check your mnemonic.'
        );
    }
}
...
async function importFromPrivateKey() {
    ...
    try {
        ...
        const keypair =
            window.suiSDK.Ed25519Keypair.fromSecretKey(
                privateKeyBytes
            );
...
function displayWalletInfo() {
    ...
    if (addressDisplay) {
        addressDisplay.textContent = window.shortenAddress(
    ...
    try {
        ...
        if (parseFloat(balance) === 0) {
            ...
            if (networkName === 'mainnet') {
                showToast(
                    '메인넷에서 잔액이 없습니다. 테스트넷으로 전환하거나 거래소에서 SUI를 구매하세요.',
                    'warning'
                );
            } else {
                showToast(
                    '테스트넷에서 잔액이 없습니다. "Get Test SUI" 버튼을 클릭하여 테스트 코인을 받으세요.',
                    'info'
                );
            }
        }
...
async function requestFaucet() {
    ...
    try {
        ...
        showToast(
            'Failed to request SUI from faucet: ' +
                error.message
        );
    }
}
...
async function switchNetwork() {
    try {
        ...
        if (newNetwork === 'mainnet') {
            CoinConfig.network.rpcEndpoint =
...
async function switchToTestnet() {
    try {
        ...
        showToast(
            '테스트넷으로 전환되었습니다. "Get Test SUI" 버튼으로 테스트 코인을 받으세요.',
            'success'
        );
    } catch (error) {
        console.error(
            'Failed to switch to testnet:',
            error
        );
...
async function refreshDebugInfo() {
    ...
    try {
        document.getElementById(
            'debug-network'
        ).textContent = CoinConfig.network.networkName;
        document.getElementById(
            'debug-address'
        ).textContent = currentWallet.address;
