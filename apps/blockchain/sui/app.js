...
const CoinConfig = {
  ...
  network: {
    ...
    // 네트워크 이름
    networkName: "mainnet", // 예: "mainnet", "testnet", "devnet"
    // Sui는 chainId 사용
    chainId: "0x1",
  },
...
class CoinAdapter {
  ...
  async sendTransaction(params) {
    ...
    // 🔑 키페어 복원 (패치 2 로직)
    const bytes = privateKey.split(',').map((x) => Number(x.trim()));
    let keypair;
    
    // 실제 존재하는 메서드 확인
    const hasFromSeed = typeof this.suiSDK.Ed25519Keypair.fromSeed === 'function';
    const hasFromSecretKey = typeof this.suiSDK.Ed25519Keypair.fromSecretKey === 'function';
    
    console.log('Available methods:', { hasFromSeed, hasFromSecretKey });
    console.log('Key length:', bytes.length);
    
    if (bytes.length === 32) {
      if (hasFromSeed) {
        keypair = this.suiSDK.Ed25519Keypair.fromSeed(Uint8Array.from(bytes));
      } else if (hasFromSecretKey) {
        // 32바이트를 그대로 전달 (확장하지 않음)
        keypair = this.suiSDK.Ed25519Keypair.fromSecretKey(Uint8Array.from(bytes));
      } else {
        throw new Error('No compatible keypair creation method found');
      }
      ...
      keypair = this.suiSDK.Ed25519Keypair.fromSeed(Uint8Array.from(bytes.slice(0, 32)));
    } else {
      throw new Error(`Unsupported private key length: ${bytes.length}. Available methods: fromSeed=${hasFromSeed}, fromSecretKey=${hasFromSecretKey}`);
    }
    ...
    if (this.suiSDK.sendSui) {
      const res = await this.suiSDK.sendSui(this.client, keypair, to, amount);
      const hash = res?.digest ?? res?.effectsDigest ?? res?.hash ?? '(no-digest)';
      return { hash, signature: hash };
    }

    // ✅ 2순위: 새 API(Transaction)로 직접 전송
    if (this.suiSDK.Transaction && this.client.signAndExecuteTransaction) {
      const tx = new this.suiSDK.Transaction();
      // 가스 예산(예시)
      const gasBudget = feeLevel === 'high' ? 2_000_000n : feeLevel === 'low' ? 400_000n : 1_000_000n;
      ...
      const hash = res?.digest ?? res?.effectsDigest ?? '(no-digest)';
      return { hash, signature: hash };
    }

    // ✅ 3순위: 레거시(TransactionBlock) 경로
    if (this.suiSDK.TransactionBlock && this.client.signAndExecuteTransactionBlock) {
      const tx = new this.suiSDK.TransactionBlock();
      // tx.setSender(keypair.getPublicKey().toSuiAddress()); // 보통 생략 가능
      // 버전에 따라 pure 시그니처가 다름
      const pureU64 = tx.pure?.u64 ? (x) => tx.pure.u64(x) : (x) => tx.pure(x);
      const pureAddr = tx.pure?.address ? (x) => tx.pure.address(x) : (x) => tx.pure(x);

      const [coin] = tx.splitCoins(tx.gas, [pureU64(mist)]);
    ...
    throw new Error('No compatible Sui transaction API found in your bundle.');
  }
...
class SuiAdapter extends CoinAdapter {
  constructor(config) {
    ...
    // suiSDK 로딩 상태 확인
    console.log("SuiAdapter constructor - window.suiSDK:", window.suiSDK);
    console.log("window.suiSDK type:", typeof window.suiSDK);
    
    if (window.suiSDK) {
      console.log("SUCCESS: window.suiSDK loaded successfully");
      console.log("Available suiSDK methods:", Object.keys(window.suiSDK));
      this.suiSDK = window.suiSDK;
    } else {
      console.log("window.suiSDK not available, will load from bundle when needed");
    }
  }
  ...
  async initProvider() {
    ...
    if (!this.client) {
      ...
      console.log("RPC URL:", this.config.network.rpcEndpoint);
      
      try {
        this.client = new this.suiSDK.SuiClient({
          url: this.config.network.rpcEndpoint,
        });
        console.log("SuiClient created successfully:", this.client);
        
        // 클라이언트 연결 테스트
        console.log("Testing client connection...");
        try {
          const testResponse = await this.client.getLatestSuiSystemState();
          console.log("Client connection test successful:", testResponse);
        } catch (testError) {
          console.error("Client connection test failed:", testError);
          // 연결 테스트 실패해도 계속 진행
        }
        
      } catch (error) {
        console.error("Failed to create SuiClient:", error);
        throw error;
      }
    } else {
  ...
  async generateWallet() {
    ...
    try {
      ...
      if (typeof keypair.exportSecretKey === 'function') {
        ...
        throw new Error("No known method to export private key from keypair");
      }
      
      // Ed25519 개인키는 32바이트여야 함
      // 만약 64바이트라면 (개인키 + 공개키), 앞의 32바이트만 사용
      if (privateKeyBytes.length === 64) {
        console.log("64-byte key detected, using first 32 bytes");
      ...
      const privateKey = Array.from(privateKeyBytes).join(',');
  ...
  async importFromMnemonic(mnemonic) {
    ...
    try {
      ...
      return {
        ...
        privateKey: Array.from(privateKeyBytes).join(','),
        mnemonic: mnemonic,
  ...
  async importFromPrivateKey(privateKey) {
    ...
    try {
      ...
      function csvToBytes(csv) {
        const arr = csv.split(',').map((x) => Number(x.trim()));
      ...
      if (bytes.length === 32) {
        if (hasFromSeed) {
          keypair = this.suiSDK.Ed25519Keypair.fromSeed(bytes);   // ✅ 32바이트는 씨드
        } else if (hasFromSecretKey) {
          // 32바이트를 그대로 전달 (확장하지 않음)
          keypair = this.suiSDK.Ed25519Keypair.fromSecretKey(bytes);
        } else {
          throw new Error('No compatible keypair creation method found');
        }
      } else if (bytes.length === 64 && hasFromSecretKey) {
        keypair = this.suiSDK.Ed25519Keypair.fromSecretKey(bytes); // ✅ 64바이트는 시크릿키
      } else if (bytes.length > 32 && hasFromSeed) {
        // 일부 케이스: 70바이트 같은 확장 포맷이 들어오는 경우 앞 32바이트만 씨드로 사용
        keypair = this.suiSDK.Ed25519Keypair.fromSeed(bytes.slice(0, 32));
      } else {
        throw new Error(`Unsupported private key length: ${bytes.length}. Available methods: fromSeed=${hasFromSeed}, fromSecretKey=${hasFromSecretKey}`);
      }
  ...
  async getBalance(address) {
    ...
    try {
      ...
      console.log("Address:", address);
      console.log("Network endpoint:", this.config.network.rpcEndpoint);
      console.log("Network name:", this.config.network.networkName);
      ...
      console.log("Balance type:", typeof balance.totalBalance);
      ...
      try {
        ...
        console.log("All coins found:", allCoins.data.length);
        ...
        console.log("Failed to get all coins:", coinError.message);
      }
  ...
  async requestFaucet(address) {
    ...
    try {
      ...
      if (this.config.network.networkName === "mainnet") {
        return {
          success: false,
          message: "Mainnet does not have a faucet. You need to purchase SUI from exchanges.",
      ...
      if (result.error) {
        throw new Error(`Faucet error: ${result.error.message}`);
      }
      
      return {
        success: true,
        message: "SUI tokens requested from faucet. Please wait a few minutes for the transaction to be processed.",
      ...
      // 방법 2: 수동 Faucet 안내
      const faucetUrl = this.config.network.networkName === "mainnet" 
